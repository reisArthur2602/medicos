    ...
    try:
        if ext == ".pdf":
            paginas = convert_from_path(
                papel_timbrado_path, dpi=300, size=(int(largura), int(altura))
            )
            img_bg = paginas[0]
            ...
            with Image.open(papel_timbrado_path) as bg:
                bg = bg.convert("RGB").resize(
                    (int(largura), int(altura)), Image.LANCZOS
                )
                temp_bg_path = os.path.join(
                    tempfile.gettempdir(), "tmp_papel_timbrado.png"
                )
                bg.save(temp_bg_path, quality=95)
                pdf.drawImage(
                    temp_bg_path, 0, 0, width=largura, height=altura, mask="auto"
                )
                os.remove(temp_bg_path)
...
# ------------------------------------------------------------
def desenhar_texto_multilinha(
    pdf, texto, x, y, largura_caixa, fontname="Helvetica", fontsize=11, leading=15
):
    ...
    try:
        ...
        if _table_exists(cur, "clinica_config"):
            for k in ("DEFAULT_PAPER_RECEITA", "PAPER_RECEITA", "paper_receita"):
                try:
                    cur.execute(
                        "SELECT valor FROM clinica_config WHERE chave=%s LIMIT 1", (k,)
                    )
                    row = cur.fetchone()
...
def gerar_receita():
    ...
    cert_path, cert_senha, assinatura_img_path, crm_medico, nome_medico = (
        obter_dados_medico_basico(medico_id)
    )
    ...
    try:
        conn = _db()
        paciente_id = _get_or_create_paciente(
            conn, nome_paciente, cpf_paciente, nasc_sql, sexo
        )
        cur = conn.cursor()
        cur.execute(
            """
        ...
        """,
            (
                ...
                "TEMP",
                data_emissao_dt.strftime("%Y-%m-%d %H:%M:%S"),
                1,
            ),
        )
    ...
    try:
        ...
        # corpo no modelo do bloco (usado em AMBAS as vias quando controlada)
        def draw_body_controlada_modelo(pdf_canvas, start_y):
            ...
            y = _rotulo_e_linha(
                ...
                largura_texto,
                dx=70,
                lh=18 if tamanho_papel == "A4" else 16,
            )
            y = _rotulo_e_linha(
                pdf_canvas,
                margem_x,
            ...
            _linha(
                pdf_canvas,
                cxr - (240 if tamanho_papel == "A4" else 210),
                y - 2,
                230 if tamanho_papel == "A4" else 200,
            )
            ...
            # ESQUERDA: Comprador (linhas separadas)
            yy = top_y - 18
            pdf_canvas.setFont("Helvetica-Bold", LP["body_font"])
            pdf_canvas.drawString(left_x + 10, yy, "IDENTIFICAÇÃO DO COMPRADOR")
            yy -= 18
            pdf_canvas.setFont("Helvetica", LP["small_font"])
            pdf_canvas.drawString(left_x + 10, yy, "Nome:")
            _linha(pdf_canvas, left_x + 10 + 45, yy - 2, col_w - 20 - 45)
            yy -= 18
            pdf_canvas.drawString(left_x + 10, yy, "Identidade:")
            _linha(pdf_canvas, left_x + 10 + 60, yy - 2, col_w - 20 - 60)
            yy -= 18
            pdf_canvas.drawString(left_x + 10, yy, "Órg.Em.:")
            _linha(pdf_canvas, left_x + 10 + 55, yy - 2, col_w - 20 - 55)
            yy -= 18
            pdf_canvas.drawString(left_x + 10, yy, "Endereço:")
            _linha(pdf_canvas, left_x + 10 + 60, yy - 2, col_w - 20 - 60)
            yy -= 18
            pdf_canvas.drawString(left_x + 10, yy, "Cidade:")
            _linha(pdf_canvas, left_x + 10 + 45, yy - 2, col_w - 20 - 45)
            yy -= 18
            pdf_canvas.drawString(left_x + 10, yy, "UF:")
            _linha(pdf_canvas, left_x + 10 + 20, yy - 2, 40)
            yy -= 18
            pdf_canvas.drawString(left_x + 10, yy, "Telefone:")
            _linha(pdf_canvas, left_x + 10 + 55, yy - 2, col_w - 20 - 55)
            ...
            yy2 -= 18
            pdf_canvas.setFont("Helvetica", LP["small_font"])
            pdf_canvas.drawString(right_x + 10, yy2, "Nome:")
            _linha(pdf_canvas, right_x + 10 + 45, yy2 - 2, col_w - 20 - 45)
            yy2 -= 18
            pdf_canvas.drawString(right_x + 10, yy2, "CNPJ:")
            _linha(pdf_canvas, right_x + 10 + 40, yy2 - 2, col_w - 20 - 40)
            yy2 -= 18
            pdf_canvas.drawString(right_x + 10, yy2, "Endereço:")
            _linha(pdf_canvas, right_x + 10 + 60, yy2 - 2, col_w - 20 - 60)
            yy2 -= 18
            pdf_canvas.drawString(right_x + 10, yy2, "Cidade:")
            _linha(pdf_canvas, right_x + 10 + 45, yy2 - 2, col_w - 20 - 45)
            yy2 -= 18
            pdf_canvas.drawString(right_x + 10, yy2, "Telefone:")
            _linha(pdf_canvas, right_x + 10 + 55, yy2 - 2, col_w - 20 - 55)
            yy2 -= 22

            pdf_canvas.setFont("Helvetica-Bold", LP["body_font"])
            pdf_canvas.drawString(right_x + 10, yy2, "ASSINATURA DO FARMACÊUTICO")
            yy2 -= 18
            pdf_canvas.setFont("Helvetica", LP["small_font"])
            pdf_canvas.drawString(right_x + 10, yy2, "Assinatura:")
            _linha(pdf_canvas, right_x + 10 + 70, yy2 - 2, col_w - 20 - 70)
            yy2 -= 18
        ...
        # corpo simples para NÃO controlada (preserva enters)
        def draw_body_simples(pdf_canvas, start_y):
            ...
            pdf_canvas.drawCentredString(
                cx, y_linha - 12, "Assinatura e carimbo do médico"
            )
        ...
        # -------- Geração das páginas
        if receita_controlada:
            # VIA 1 (idêntica ao modelo da via 2)
            ...
            # Receita comum: apenas uma página simples
    ...
    has_cert_inputs = bool(
        cert_path and os.path.isfile(cert_path) and (cert_senha or "").strip()
    )
    if has_cert_inputs:
        try:
            cmd = [
            ...
            if proc.returncode == 0:
                ...
                if not any(os.path.exists(p) for p in candidates):
                    candidates = glob.glob(
                        os.path.join(os.path.dirname(base_path), "*_signed.pdf")
    ...
    if assinou:
        try:
            base_url = (
                PUBLIC_BASE_URL if PUBLIC_BASE_URL else request.url_root.rstrip("/")
            )
            ...
            ov.drawImage(
                ImageReader(gerar_qrcode(url_validacao)),
                qr_x,
                qr_y,
                qr_size,
                qr_size,
            ...
            if (
                assin_path
                and os.path.exists(assin_path)
                and assin_path.lower().endswith((".png", ".jpg", ".jpeg"))
            ):
                try:
                    with Image.open(assin_path) as img:
                        ...
                        tmp_ass = os.path.join(
                            tempfile.gettempdir(), "assinatura_tmp_receita.png"
                        )
                    ...
                    ov.drawImage(
                        tmp_ass, x_ass, y_ass, width=nw, height=nh, mask="auto"
                    )
                    try:
                        os.remove(tmp_ass)
                    except Exception:
            ...
            ov.drawString(
                text_x, linha_centro_y + 15, f"{nome_medico}    |    {conselho_label}"
            )
            ov.setFont("Helvetica", 9)
            ov.drawString(
                text_x,
                linha_centro_y,
                "Para verificar a autenticidade da receita, leia o QR code ao lado.",
            )
            ov.drawString(
                text_x,
                linha_centro_y - 15,
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "UPDATE receitas SET pdf_assinado_path=%s WHERE id=%s",
